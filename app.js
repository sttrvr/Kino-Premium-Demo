...
// Load comment badges lazily only for cards entering the viewport (limits network work)
let __commentBadgeIO = null;
const __commentBadgeFetched = new Set();
function updateCommentBadges(movies){
  try {
    if (typeof window === 'undefined' || !window.Comments) return;
    if (!elements.moviesGrid) return;
    // Create IO once
    if (!__commentBadgeIO) {
      __commentBadgeIO = new IntersectionObserver(async (entries, obs) => {
        for (const entry of entries) {
          if (!entry.isIntersecting) continue;
          const card = entry.target;
          const id = Number(card.dataset.id);
          if (!id || __commentBadgeFetched.has(id)) { obs.unobserve(card); continue; }
          __commentBadgeFetched.add(id);
          try {
            const count = await window.Comments.getCount(id);
            const meta = card.querySelector('.meta');
            if (meta) {
              let badge = meta.querySelector('.comment-badge');
              if (!badge) {
    ...
    // Observe only up to N new cards per render
    const MAX_OBSERVE = 12;
    let observed = 0;
    elements.moviesGrid.querySelectorAll('.movie-card').forEach(card => {
      const id = Number(card.dataset.id);
      if (!id || __commentBadgeFetched.has(id)) return;
      if (observed >= MAX_OBSERVE) return;
      __commentBadgeIO.observe(card);
      observed++;
    });
  } catch {}
}
...
function pauseCarouselAutoplay(delayMs = 3000) {
  clearInterval(state.carouselInterval);
  if (state.carouselResumeTimeout) { clearTimeout(state.carouselResumeTimeout); }
  state.carouselResumeTimeout = setTimeout(() => {
    state.carouselResumeTimeout = null;
...
function setupEventListeners() {
  ...
  if (elements.footerHistory) {
    elements.footerHistory.addEventListener('click', (e)=>{
      e.preventDefault();
      if (elements.moviesSectionTitle) elements.moviesSectionTitle.textContent = 'Tarix';
      showHistory();
    });
  }
  if (elements.footerProfile) {
    elements.footerProfile.addEventListener('click', (e)=>{
      e.preventDefault();
      const modal = document.getElementById('authModal');
      if (modal) modal.removeAttribute('hidden'), modal.setAttribute('aria-hidden','false');
    });
  }
  if (elements.footerSettings) {
    elements.footerSettings.addEventListener('click', (e)=>{
      e.preventDefault();
      openSettingsModal();
    });
  }
  ...
  const btnLight = document.getElementById('settingsThemeLight');
  const btnDark = document.getElementById('settingsThemeDark');
  ...
  if (clearFav) clearFav.addEventListener('click',()=>{
    ...
    state.favorites = [];
    alert('Sevimlilar tozalandi.');
  });
  if (clearHist) clearHist.addEventListener('click',()=>{
    try { localStorage.removeItem('watch_history'); } catch {}
...
function setTheme(theme){
  ...
  if (metaTheme) metaTheme.setAttribute('content', theme === 'dark' ? '#0b0b0f' : '#ffffff');
}
